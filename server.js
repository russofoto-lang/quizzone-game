...
zoomMode: {
active: false,           // Se zoom progressivo è attivo
currentZoom: null,       // Immagine corrente
currentLevel: 1,         // Livello zoom corrente (1-5)
pointsAvailable: 250,    // Punti disponibili ora
startTime: 0,            // Quando è partito
alreadyAnswered: []      // Team che hanno già risposto (e sbagliato)
},
memoryMode: {
active: false,           // Se memory è attivo
level: 1,                // Livello 1=8carte, 2=12carte, 3=16carte
cards: [],               // Array delle carte [emoji1, emoji2, …]
...
phase: ‘memorize’        // ‘memorize’ | ‘covered’ | ‘answer’ | ‘result’
}
};
...
socket.on(‘zoom_start’, (data) => {
console.log(‘🔍 Avvio Zoom Progressivo:’, data.zoom.nome);
...
console.log('🔍 Zoom livello 1 - 250pt disponibili');
```
...
console.log(‘🔍 Zoom livello 2 - 200pt disponibili’);
}
}, 3000);
...
setTimeout(() => {
  if(gameState.zoomMode.active && gameState.zoomMode.currentLevel === 2) {
    ...
    console.log('🔍 Zoom livello 3 - 150pt disponibili');
  }
}, 6000);
...
setTimeout(() => {
  if(gameState.zoomMode.active && gameState.zoomMode.currentLevel === 3) {
    ...
    console.log('🔍 Zoom livello 4 - 100pt disponibili');
  }
}, 9000);
...
setTimeout(() => {
  if(gameState.zoomMode.active && gameState.zoomMode.currentLevel === 4) {
    ...
    console.log('🔍 Zoom livello 5 - 50pt disponibili (immagine completa)');
  }
}, 12000);
```
...
socket.on(‘zoom_buzzer_press’, (data) => {
...
// Controlla se ha già risposto (e sbagliato)
if(gameState.zoomMode.alreadyAnswered.includes(teamId)) {
  console.log('⚠️ Squadra già eliminata:', team.name);
  return;
}
...
console.log('🔍 Buzzer premuto da:', team.name, 'al livello', gameState.zoomMode.currentLevel, '('+gameState.zoomMode.pointsAvailable+'pt)');
```

});

socket.on(‘zoom_answer_result’, (data) => {
if(!gameState.zoomMode.active) return;
...
if(isCorrect) {
  ...
  console.log('🔍 ✅ CORRETTO!', team.name, 'vince', points, 'pt al livello', gameState.zoomMode.currentLevel);
  ...
  console.log('🔍 ❌ SBAGLIATO!', team.name, 'eliminato');
  ...
  setTimeout(() => {
    if(gameState.zoomMode.active) {
      gameState.buzzerLocked = false;
      io.emit('stato_buzzer', { locked: false, attiva: true });
      console.log('🔍 Buzzer riaperto per gli altri');
    }
  }, 2000);
}
```
...
console.log(‘🧠 Avvio Memory Snapshot - Livello’, level);
...
console.log('🧠 Fase MEMORIZZA -', timing.memorize, 'secondi');
...
gameState.memoryMode.phase = ‘covered’;

```
io.emit('memory_cover_all');

console.log('🧠 Carte coperte');
...
gameState.memoryMode.phase = ‘answer’;
...
console.log('🧠 Carta rivelata:', gameState.memoryMode.revealedImage, 'in pos', gameState.memoryMode.revealedPosition);
console.log('🧠 Risposta corretta:', gameState.memoryMode.correctPosition);
...
gameState.memoryMode.phase = ‘result’;
...
Object.entries(gameState.memoryMode.answers).forEach(([teamId, answer]) => {
  ...
  if(isCorrect) {
    ...
    console.log('🧠 ✅', team.name, 'CORRETTO! +' + points + 'pt');
  } else {
    results.push({
      teamId: teamId,
      teamName: team.name,
    ...
    console.log('🧠 ❌', team.name, 'sbagliato (risposta:', answer, ')');
  }
});
...
if(gameState.memoryMode.phase !== ‘answer’) return;

```
gameState.memoryMode.answers[data.teamId] = data.position;
console.log('🧠 Risposta da', gameState.teams[data.teamId]?.name, '→ posizione', data.position);
```
